#!/usr/bin/env node
/**
 * Consolidated release preparation script.
 *
 * This script:
 ...
 * - ALL_RELEASE: "inherit" (default), "minor", or "patch"
...
function resolveReleaseConfig() {
  ...
  const ALL_RELEASE = validateInput('ALL_RELEASE', process.env.ALL_RELEASE || 'inherit');
  const SEMCONV_RELEASE = validateInput('SEMCONV_RELEASE', process.env.SEMCONV_RELEASE || 'inherit');

  // Check for conflicting configuration
  if (isSet(ALL_RELEASE) && (isSet(STABLE_SDK_RELEASE) || isSet(EXPERIMENTAL_RELEASE))) {
    console.error('Error: ALL_RELEASE cannot be set when STABLE_SDK_RELEASE or EXPERIMENTAL_RELEASE are also set.');
    console.error('Please use either ALL_RELEASE or individual package group settings, not both.');
    ...
    console.error(`  ALL_RELEASE: ${ALL_RELEASE}`);
    process.exit(1);
  }
  ...
  if (isSet(ALL_RELEASE)) {
    // ALL_RELEASE overrides everything except semconv
    releaseTypeApi = ALL_RELEASE;
    releaseTypeStable = ALL_RELEASE;
    releaseTypeExperimental = ALL_RELEASE;
  } else if (isSet(STABLE_SDK_RELEASE)) {
    // Stable SDK release
    releaseTypeStable = STABLE_SDK_RELEASE;
    // Experimental inherits from stable SDK if not explicitly set
    releaseTypeExperimental = isSet(EXPERIMENTAL_RELEASE) ? EXPERIMENTAL_RELEASE : STABLE_SDK_RELEASE;
  ...
  if (!releaseTypeApi && !releaseTypeStable && !releaseTypeExperimental && !releaseTypeSemconv) {
    ...
    console.error('At least one of STABLE_SDK_RELEASE, EXPERIMENTAL_RELEASE, ALL_RELEASE, or SEMCONV_RELEASE must be set to "minor" or "patch".');
    process.exit(1);
  }
...
// Main execution
function main() {
  console.log('=== OpenTelemetry JS Release Preparation ===\n');
  ...
  console.log('  2. Commit with: git commit -am "chore: prepare release"');
