...
describe('MetricCollector', () => {
  ...
  describe('collect', () => {
    ...
    it('sync + cumulative: should persist data points across empty cycles with repeating start timestamps', async () => {
      ...
      const dp1 = result1.resourceMetrics.scopeMetrics[0].metrics[0].dataPoints[0];
      ...
      const dp2 = result2.resourceMetrics.scopeMetrics[0].metrics[0].dataPoints[0];
      assertDataPoint(dp2, { key: 'A' }, 5);
      // MUST repeat the same start timestamp
      assert.deepStrictEqual(dp2.startTime, startTime1,
      ...
      const dps3 = result3.resourceMetrics.scopeMetrics[0].metrics[0].dataPoints;
      ...
      assert.deepStrictEqual(dps3[0].startTime, startTime1,
      ...
      const dps4 = result4.resourceMetrics.scopeMetrics[0].metrics[0].dataPoints;
      assert.strictEqual(dps4.length, 2, 'both A and B must persist in cumulative even with no new recordings');
      assertDataPoint(dps4[0], { key: 'A' }, 8);
      assertDataPoint(dps4[1], { key: 'B' }, 7);
      assert.deepStrictEqual(dps4[0].startTime, startTime1, 'A startTime must repeat');
      assert.deepStrictEqual(dps4[1].startTime, dps3[1].startTime, 'B startTime must repeat after its first appearance');
    });
    ...
    it('sync + delta: should only report when measurements recorded, with advancing start timestamps', async () => {
      ...
      const dps1Delta = result1.resourceMetrics.scopeMetrics[0].metrics[0].dataPoints;
      ...
      assert.strictEqual(result2.resourceMetrics.scopeMetrics.length, 0,
      ...
      const result3 = await collector.collect();
      assert.strictEqual(result3.resourceMetrics.scopeMetrics.length, 1);
      const dps3Delta = result3.resourceMetrics.scopeMetrics[0].metrics[0].dataPoints;
      assert.strictEqual(dps3Delta.length, 2, 'both A and B must re-appear after recording');
      ...
      assert.ok(
        dps3Delta[0].startTime[0] > endTime1[0] ||
          (dps3Delta[0].startTime[0] === endTime1[0] && dps3Delta[0].startTime[1] >= endTime1[1]),
    ...
    it('async + cumulative: should only report observed attribute sets, with repeating start timestamps', async () => {
      ...
      const dpA1 = (metrics1.dataPoints as Array<{ attributes: Record<string, unknown>; startTime: [number, number] }>).find(dp => dp.attributes['key'] === 'A')!;
      ...
      assert.strictEqual(metrics2.dataPoints.length, 1,
      ...
      assert.deepStrictEqual(metrics2.dataPoints[0].startTime, startTimeA,
      ...
      assert.strictEqual(metrics3.dataPoints.length, 2, 'both A and B must re-appear when observed again');
      ...
      assert.deepStrictEqual(metrics3.dataPoints[0].startTime, startTimeA,
        'A startTime must keep repeating after B re-appearance');
      const dpB1 = (metrics1.dataPoints as Array<{ attributes: Record<string, unknown>; startTime: [number, number] }>).find(dp => dp.attributes['key'] === 'B')!;
      assert.deepStrictEqual(metrics3.dataPoints[1].startTime, dpB1.startTime,
      ...
      clock.tick(10);
      const result4 = await collector.collect();
      assert.strictEqual(result4.resourceMetrics.scopeMetrics.length, 0,
    ...
    it('async + delta: should only report observed attribute sets, with advancing start timestamps', async () => {
      ...
      assert.deepStrictEqual(dp2.startTime, endTime1,
