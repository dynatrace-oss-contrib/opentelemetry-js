...
#### Use Case: Integration with Node.js TracingChannel

The `attach()` and `detach()` methods are particularly useful when integrating with Node.js's [`TracingChannel` API](https://nodejs.org/api/diagnostics_channel.html#tracingchannel) (available in Node.js v19.9.0+). TracingChannel provides synchronous event callbacks (`start`, `end`, `asyncStart`, `asyncEnd`, `error`) where you need to imperatively set and restore context:
...
channel.start.bindStore({}, {
  start(message) {
    // Attach context at operation start, preserving existing context
    const ctx = contextManager.active().setValue('operationId', message.id);
    const token = contextManager.attach(ctx);
    tokens.set(message, token);
  },
  end(message) {
    // Restore context at operation end
...
Before `attach()`/`detach()`, there was no good way to set context in these synchronous callbacks because `with()` would restore the context immediately after the callback returns, before any async work could execute. See [examples/tracing-channel-example.js](./examples/tracing-channel-example.js) for a complete working example.
