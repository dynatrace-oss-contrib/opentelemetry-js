...
describe('PrometheusExporter', () => {
  ...
  describe('export', () => {
    ...
    it('should export a count aggregation', done => {
      ...
      const boundCounter = counter.bind({ key1: 'attributeValue1' });
      boundCounter.add(10);
      meter.collect().then(() => {
    ...
    it('should export multiple attributes', done => {
      ...
      counter.bind({ counterKey1: 'attributeValue1' }).add(10);
      counter.bind({ counterKey1: 'attributeValue2' }).add(20);
      meter.collect().then(() => {
        exporter.export(meter.getProcessor().checkPointSet(), () => {
          http
            .get('http://localhost:9464/metrics', res => {
    ...
    it('should export multiple attributes on manual shutdown', done => {
      ...
      counter.bind({ counterKey1: 'attributeValue1' }).add(10);
      counter.bind({ counterKey1: 'attributeValue2' }).add(20);
      counter.bind({ counterKey1: 'attributeValue3' }).add(30);
          ...
          .get('http://localhost:9464/metrics', res => {
            res.on('data', chunk => {
              ...
              assert.deepStrictEqual(lines, [
                ...
                `counter_total{counterKey1="attributeValue1"} 10 ${mockedHrTimeMs}`,
                `counter_total{counterKey1="attributeValue2"} 20 ${mockedHrTimeMs}`,
                `counter_total{counterKey1="attributeValue3"} 30 ${mockedHrTimeMs}`,
                '',
              ]);
    ...
    it('should export a UpDownCounter as a gauge', done => {
      ...
      counter.bind({ key1: 'attributeValue1' }).add(20);
      meter.collect().then(() => {
        exporter.export(meter.getProcessor().checkPointSet(), () => {
          http
            .get('http://localhost:9464/metrics', res => {
    ...
    it('should export a Histogram as a summary', done => {
      ...
      histogram.bind({ key1: 'attributeValue1' }).record(20);
  ...
  describe('configuration', () => {
    ...
    beforeEach(() => {
      ...
      counter.bind({ key1: 'attributeValue1' }).add(10);
    });
